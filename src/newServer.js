 ...
 _initServices() {
   this.server.addService(proximaProto.ProximaService.service, {
       create: (call, callback) => {
         ...
         this.db.create(request.name).then(function(response) {
           let reply = {
             confirmation: true
           }
           callback(null, reply)
       ...
       open: (call, callback) => {
         let request = call.request
         this.db.open(request.name).then(function(response) {
           let reply = {
             confirmation: true
           }
           callback(null, reply)
       ...
       close: (call, callback) => {
         let request = call.request
         this.db.close(request.name).then(function(response) {
          let reply = {
             confirmation: true
           }
           callback(null, reply)
       ...
       remove: async (call, callback) => {
         let request = call.request
         let table = await this.db.get(request.name)
         table.remove(request.key, request.prove).then(function(response) {
           let reply = {
             root: response.root,
             proof: parseProof(response.proof)
           }
       ...
       scan: async (call, callback) => {
         let request = call.request
         let table = await this.db.get(request.name)
         let response = await table.scan(request.first, request.last, request.limit, request.prove).then(function(responses) {
           let reply = new Array()
            for (var response in responses) {
              reply.push({
                value: response.value,
       ...
       query: async (call, callback) => {
         let request = call.request
         let table = await this.db.get(request.name)
         table.query(request.query, request.prove).then(function(responses) {
           let reply = new Array()
            for (var response in responses) {
              reply.push({
                value: response.value,
